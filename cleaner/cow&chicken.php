<?php
/*********************************
code to clean strings and arrays *
 *********************************/
...
// we need to split the above strings
// string- we shall explode
// explode ( DELIMETER, HAYSTACK/BIG STRING);
// returns an array with split string
//$jj = "k.m.1.2.3.34";
// <?php
//   $str=ucwords(strtolower($wce_array [$i]));
  //$str = preg_replace("/" . [a-Z/A-z] . "/", "<b>" . $search_string . "</b>", $result['Country_Name'])
//var var_dump(PARAM);
//var dump ('.', $jj)
//);
//echo $wambua_corrupted_employees;
//<?php
$cow = explode (',', stringtoupper($wambua_corrupted_employee));
$chicken = array_splice(array_unique($cow),0,8);
...
;
$cow = explode (',', stringtoupper($wambua_corrupted_employee));
$chicken = array_splice(array_unique($cow),8,254);
$array_of_rows = array_chunk($cow&chicken, 8);
for( $array_of_rows as $cells) {
  echo '<tr>';
  $i = 1 ;
